...
def analyze_and_report(df: pd.DataFrame) -> Tuple[BytesIO, BytesIO]:
    """执行完整的数据分析和报表生成"""
    config = load_config()
    ...
    # 检测异常值
    from cleaning import detect_anomalies
    anomalies = detect_anomalies(df, config)
    ...
    return excel_report, visualization 