...
class DouyinAnalyzer:
    ...
    def generate_excel_report(self):
        ...
        """
        # 创建Excel写入器
        excel_path = os.path.join(self.output_dir, f'douyin_report_{self.timestamp}.xlsx')
        ...
        if summary is not None:
            summary.to_excel(writer, sheet_name='描述统计')
        
        # 创建销量分析表
        if '销量区间' in self.df.columns:
            sales_pivot = pd.pivot_table(
                self.df,
                values='近30天销量_清洗',
                index='销量区间',
                aggfunc={
                    '近30天销量_清洗': ['count', 'sum', 'mean', 'median', 'min', 'max']
                }
            )
            sales_pivot.to_excel(writer, sheet_name='销量分析')
        
        # 创建佣金分析表
        if '佣金区间' in self.df.columns:
            commission_pivot = pd.pivot_table(
                self.df,
                values='佣金比例_清洗',
                index='佣金区间',
                aggfunc={
                    '佣金比例_清洗': ['count', 'mean', 'median', 'min', 'max']
                }
            )
            commission_pivot.to_excel(writer, sheet_name='佣金分析')
        
        # 保存Excel - 修改这里，使用close()而不是save()
        writer.close()
